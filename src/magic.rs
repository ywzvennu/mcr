...
//!
//! Direct `unsafe { *attacks.get_unchecked(index) }` was measured separately (the
//! index is `< attacks.len()` by construction, so it would be sound). In an
//! isolated micro-benchmark — tens of millions of rook+bishop reads over random
