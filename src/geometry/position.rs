...
impl<G: Geometry, V: WideVariant<G>> GenericPosition<G, V> {
    ...
    fn gen_multi_royal_pseudo<S: WideSink>(&self, pseudo: &mut S, us: Color) {
        ...
        for role in WideRole::ALL {
            // The Hoplite (Berolina) is always handled by its own emitter below.
            // The standard `Pawn` is normally handled by the straight-push pawn
            // generator below too — unless the variant's pawn is a **forward
            // stepper this generator must itself emit** (the Shogi pawn in Sho
            // Shogi: one square forward, capturing straight ahead, promoting in the
            // zone), in which case it flows through this role loop like any other
            // promotable stepper. A hand variant's stepper pawn is generated by the
            // hand path instead (this generator is shared with the cannon-verify
            // path, where hand+cannon variants like Synochess keep ordinary
            // straight-push pawns), so the routing requires `!has_hand()` — making
            // it true only for a variant that *explicitly* sets a stepper pawn with
            // no hand (Sho Shogi). Every other variant (whose `pawn_is_stepper()` is
            // the `has_hand()` default) keeps the pawn out of this loop and is
            // byte-identical.
            if role == WideRole::Hoplite {
                continue;
            }
            if role == WideRole::Pawn && (!V::pawn_is_stepper() || V::has_hand()) {
                continue;
            }
            ...
            for from in board.pieces(us, role) {
                ...
                if promotable {
                    ...
                    // forced promotion). For Chak (mandatory promotion, no forced
                    // cases) this is byte-identical to the simpler emitter it
                    // replaces.
        ...
        // stepper this generator emits itself (Sho Shogi: `pawn_is_stepper()` with
        // no hand) — it was already produced via the role loop above. A hand+cannon
        // variant (Synochess) keeps its standard pawns here.
        if !V::pawn_is_stepper() || V::has_hand() {
