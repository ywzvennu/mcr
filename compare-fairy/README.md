# mcr-compare-fairy — perft comparison vs Fairy-Stockfish

A correctness + throughput harness that compares the `mcr` chess rules library's perft
...
(mcr Mn/s vs FSF Mn/s). FSF is the **oracle** for the fairy-variants work
(issue #158); this harness validates the variants mcr already shares with it.

## ⚠️ GPL fence (important)

Fairy-Stockfish is licensed **GPL-3.0-or-later**. This harness keeps the `mcr`
...
- This is a **separate, nested crate** (`compare-fairy/`), not part of the
  published `mcr` package and not in any workspace — the same arrangement as
  `compare/` and `fuzz/`. It is `publish = false` and never distributed.
- The `mcr` library **does not depend on FSF** in any form. Confirm:

  ```sh
  cargo tree -e normal -p mcr     # no FSF / compare-fairy
...
```sh
# Locate FSF via $MCR_FSF_BIN, then PATH, then a prebuilt build/ binary:
...
# Measure mcr's magic-bitboard sliders instead of hyperbola-quintessence:
...
perft on a basket of identical positions (reused from the mcr regression tests)
at a few depths and **asserts mcr's node count equals FSF's**. On a mismatch it
...
| mcr `VariantId` | FSF `UCI_Variant` | notes |
|---|---|---|
...
| ThreeCheck | `3check` | mcr trailing `W+B` relocated to FSF's after-en-passant field |
...
— never linked, never source-copied into `mcr` or this crate. It speaks the
...
(`usermove` legality via `xboard.rs`). That walk needs mcr's own **Chu-Shogi
move generator** to compare against — and mcr currently has only the 12x12
`Chu12x12` board geometry, not a Chu-Shogi *variant*. Adding those rules is
issue #380. Until then the mcr side is gated behind the `large-shogi` cargo
feature and `--hachu` reports the oracle as **READY** instead of running an
unbacked comparison. Once #380 lands, enable it with
`cargo run --release -- --hachu --features large-shogi` and wire the mcr perft
into `hachu.rs` (the integration point is marked there).

The `mcr` library still **does not depend on HaChu** in any form (it is only a
