...
//!
//! Chak runs on mce's **generic** engine (`mce::geometry::Chak`, a
...
//! ## FEN dialect
//!
//! mce and FSF render the same Chak position with **different piece letters**. FSF
...
//! Lord). mce reuses `r`/`n`/`k` (Rook / Knight=Vulture / King) and `w`
...
/// One Chak corpus position. The FEN is mce's dialect; the FSF side translates it
/// via [`to_fsf_dialect`].
struct Case {
    label: &'static str,
    fen: &'static str,
...
/// Translates an mce-dialect Chak FEN to FSF's dialect over the **placement field
...
/// Run the Chak corpus through mce and FSF. Returns the number of mismatches
...
pub fn run(engine: &mut Engine, fsf_bin: &str, full: bool) -> usize {
    ...
    if mce_s > 0.0 && fsf_s > 0.0 {
        println!(
            "chak OVERALL: {nodes} nodes verified; mce {:.1} Mn/s vs fsf {:.1} Mn/s ({:.2}x).",
    ...
    if mismatches == 0 {
        ...
        for r in rows.iter().filter(|r| !r.matched) {
            eprintln!(
                "  MISMATCH chak/{} depth {}: mce={} fsf={}  FEN: {}",
...
/// Run one Chak position through mce's generic perft and FSF's `go perft`.
fn run_case(engine: &mut Engine, case: &Case, depth: u32) -> Result<Row, String> {
    let pos = Chak::from_fen(case.fen).map_err(|e| format!("mce rejected FEN: {e:?}"))?;
