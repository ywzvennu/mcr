...
//!
//! Grandhouse runs on mce's **generic** `u128` engine (`mce::geometry::Grandhouse`,
...
//! `$MCE_FSF_VARIANTS_INI` before checking the variant (mirroring the Shogun /
...
//! ## FEN dialect
//!
//! As with Grand, mce spells the marshal `e`/`E` (its
...
use mce::geometry::{perft as gperft, Grand10x10, Grandhouse};

use crate::uci::Engine;

/// One Grandhouse corpus position, in the **mce dialect** (marshal = `e`).
struct Case {
    label: &'static str,
    fen: &'static str,
    depth: u32,
...
/// Rewrite an mce-dialect Grandhouse FEN into the FSF dialect: the marshal's
...
/// Run the Grandhouse corpus through mce and FSF. Returns the number of mismatches
...
pub fn run(engine: &mut Engine, full: bool) -> usize {
    ...
    if ini.is_empty() {
        println!("  SKIP: set $MCE_FSF_VARIANTS_INI to an FSF variants.ini defining `grandhouse`.");
        return 0;
    }
    ...
    if mce_s > 0.0 && fsf_s > 0.0 {
        println!(
            "grandhouse OVERALL: {nodes} nodes verified; mce {:.1} Mn/s vs fsf {:.1} Mn/s ({:.2}x).",
    ...
    if mismatches == 0 {
        ...
        for r in rows.iter().filter(|r| !r.matched) {
            eprintln!(
                "  MISMATCH grandhouse/{} depth {}: mce={} fsf={}  mce FEN: {}  FSF FEN: {}",
                r.label,
                r.depth,
                r.mce_nodes,
                r.fsf_nodes,
...
/// Run one Grandhouse position through mce's generic perft and FSF's `go perft`.
fn run_case(engine: &mut Engine, case: &Case, depth: u32) -> Result<Row, String> {
    // mce side: the generic Grandhouse position over the 10x10 u128 geometry.
    let pos = Grandhouse::from_fen(case.fen).map_err(|e| format!("mce rejected FEN: {e:?}"))?;
...
mod tests {
    ...
    /// The mce -> FSF dialect rewrite swaps only the marshal's letter (including a
    ...
    fn fen_dialect_rewrites_only_the_marshal() {
        let mce = "r8r/1nbqkeabn1/pppppppppp/10/10/10/10/PPPPPPPPPP/1NBQKEABN1/R8R[Ee] w - - 0 1";
