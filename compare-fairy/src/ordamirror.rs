...
//!
//! Ordamirror runs on mce's **generic** engine (`mce::geometry::Ordamirror`, a
...
//! counts match, and reports mce-vs-FSF throughput.
//!
...
//! ## FEN dialect
//!
//! mce and FSF render the same Ordamirror position with **different horde-piece
//! letters**. FSF's `ordamirror` uses `l h a f` (Lancer = kniroo, Kheshig =
//! centaur, Archer = knibis, Falcon = the `mQcN` custom piece); mce reuses
...
//! it distinct from the bare Lancer `f`). [`to_fsf_dialect`] maps mce's letters
...
/// One Ordamirror corpus position. The FEN is mce's dialect; the FSF side
...
/// Translates an mce-dialect Ordamirror FEN to FSF's dialect. The horde letters
...
/// Run the Ordamirror corpus through mce and FSF. Returns the number of
...
pub fn run(engine: &mut Engine, fsf_bin: &str, full: bool) -> usize {
    ...
    if mce_s > 0.0 && fsf_s > 0.0 {
        println!(
            "ordamirror OVERALL: {nodes} nodes verified; mce {:.1} Mn/s vs fsf {:.1} Mn/s ({:.2}x).",
    ...
    if mismatches == 0 {
        ...
        for r in rows.iter().filter(|r| !r.matched) {
            eprintln!(
                "  MISMATCH ordamirror/{} depth {}: mce={} fsf={}  FEN: {}",
...
/// Run one Ordamirror position through mce's generic perft and FSF's `go perft`.
fn run_case(engine: &mut Engine, case: &Case, depth: u32) -> Result<Row, String> {
    let pos = Ordamirror::from_fen(case.fen).map_err(|e| format!("mce rejected FEN: {e:?}"))?;
...
mod tests {
    ...
    /// The dialect swap maps mce's horde letters to FSF's — including the
    /// two-character Falcon token `*f`/`*F` to FSF's bare `f`/`F` — and leaves the
    /// structural fields untouched.
    #[test]
    fn dialect_swap_maps_horde_letters() {
        let mce = "fwy*fkywf/8/pppppppp/8/8/PPPPPPPP/8/FWY*FKYWF w - - 0 1";
