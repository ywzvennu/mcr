...
//!
//! Mansindam runs on mce's **generic** engine (`mce::geometry::Mansindam`, a
...
//! ## FEN dialect
//!
//! mce and FSF render the same Mansindam position with **different piece tokens**.
...
/// One Mansindam corpus position. The FEN is mce's dialect; the FSF side translates
...
/// Translates an mce-dialect Mansindam FEN to FSF's dialect over the **placement
...
/// Run the Mansindam corpus through mce and FSF. Returns the number of mismatches
...
pub fn run(engine: &mut Engine, fsf_bin: &str, full: bool) -> usize {
    ...
    if mce_s > 0.0 && fsf_s > 0.0 {
        println!(
            "mansindam OVERALL: {nodes} nodes verified; mce {:.1} Mn/s vs fsf {:.1} Mn/s ({:.2}x).",
    ...
    if mismatches == 0 {
        ...
        for r in rows.iter().filter(|r| !r.matched) {
            eprintln!(
                "  MISMATCH mansindam/{} depth {}: mce={} fsf={}  FEN: {}",
...
/// Run one Mansindam position through mce's generic perft and FSF's `go perft`.
fn run_case(engine: &mut Engine, case: &Case, depth: u32) -> Result<Row, String> {
    let pos = Mansindam::from_fen(case.fen).map_err(|e| format!("mce rejected FEN: {e:?}"))?;
...
mod tests {
    ...
    fn dialect_swap_maps_mansindam_tokens() {
        let mce = "rnb**akqane/9/ppppppppp/9/9/9/PPPPPPPPP/9/ENAQK**ABNR[] w - - 0 1";
        assert_eq!(
            to_fsf_dialect(mce),
