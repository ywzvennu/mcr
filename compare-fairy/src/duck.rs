...
//! (issue #177).
//!
//! Duck chess runs on mcr's **generic** 8x8 engine (`mcr::geometry::Duck`, a
...
//! ## FEN dialect
//!
//! mcr uses the **same dialect** Fairy-Stockfish does for Duck chess: the neutral
...
//! `<piecemove>,<duckfrom><duckto>` (e.g. `a2a3,a3a2`), and mcr's
...
//! matches mcr **byte-identically** on all five corpus positions at the default
...
//! stale/differently-built FSF binary, not an mcr or harness translation bug, so
...
//! suspecting mcr or this harness.
//!
...
use mcr::geometry::{perft as gperft, Chess8x8, Duck};

use crate::uci::Engine;

/// One Duck corpus position (mcr == FSF dialect).
struct Case {
    label: &'static str,
    fen: &'static str,
    depth: u32,
...
/// Run the Duck corpus through mcr and FSF. Returns the number of mismatches
...
pub fn run(engine: &mut Engine, full: bool) -> usize {
    ...
    if mcr_s > 0.0 && fsf_s > 0.0 {
        println!(
            "duck OVERALL: {nodes} nodes verified; mcr {:.1} Mn/s vs fsf {:.1} Mn/s ({:.2}x).",
    ...
    if mismatches == 0 {
        ...
        for r in rows.iter().filter(|r| !r.matched) {
            eprintln!(
                "  MISMATCH duck/{} depth {}: mcr={} fsf={}  FEN: {}",
...
/// Run one Duck position through mcr's generic perft and FSF's `go perft`.
fn run_case(engine: &mut Engine, case: &Case, depth: u32) -> Result<Row, String> {
    // mcr side: the generic Duck position over the 8x8 geometry.
    let pos = Duck::from_fen(case.fen).map_err(|e| format!("mcr rejected FEN: {e:?}"))?;
