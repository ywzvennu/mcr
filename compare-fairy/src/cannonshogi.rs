...
//! (issue #269).
//!
//! Cannon Shogi runs on mcr's **generic** `u128` engine
//! (`mcr::geometry::CannonShogi`, a `GenericPosition<Shogi9x9, CannonShogiRules>`),
...
//! ## FEN dialect
//!
//! mcr and FSF agree on the Shogi pieces (`l n s g k r b`), the Soldier (`p`), and
//! the `[..]` holdings bracket, but spell the cannon army differently. mcr reuses
...
/// Rewrites an mcr Cannon Shogi FEN into FSF's `cannonshogi` dialect: the bare
...
pub fn fen_to_fsf(fen: &str) -> String {
    ...
    while let Some(c) = chars.next() {
        match c {
            ...
            // The bare Cannon (mcr reuses `WideRole::Cannon`) is FSF's `u`.
            'c' => out.push('u'),
            'C' => out.push('U'),
...
/// The FSF spelling of an mcr Cannon Shogi `=`-overflow base letter (case = colour).
...
vs FSF UCI_Variant cannonshogi (issue #269):"
    ...
    if mcr_s > 0.0 && fsf_s > 0.0 {
        println!(
            "cannonshogi OVERALL: {nodes} nodes verified; mcr {:.1} Mn/s vs fsf {:.1} Mn/s ({:.2}x).",
    ...
    if mismatches == 0 {
        ...
        for r in rows.iter().filter(|r| !r.matched) {
            eprintln!(
                "  MISMATCH cannonshogi/{} depth {}: mcr={} fsf={}  FEN: {}",
...
/// Run one Cannon Shogi position through mcr's generic perft and FSF's `go perft`.
fn run_case(engine: &mut Engine, case: &Case, depth: u32) -> Result<Row, String> {
    let pos = CannonShogi::from_fen(case.fen).map_err(|e| format!("mcr rejected FEN: {e:?}"))?;
...
/// Locate FSF's `variants.ini` (which defines `cannonshogi`): `$MCR_FSF_VARIANTS_INI`
...
mod tests {
    ...
    fn corpus_fens_parse_and_match_pinned_shallow_counts() {
        ...
        for case in CASES {
            ...
            let n = gperft::<Shogi9x9, _, _>(&pos, 2);
            let want = pinned
                .iter()
    ...
    /// The FEN rewriter maps the mcr cannon dialect to FSF's spelling.
    #[test]
