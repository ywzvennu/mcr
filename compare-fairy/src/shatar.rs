...
//! (issue #229).
//!
//! Shatar runs on mce's **generic** engine (`mce::geometry::Shatar`, a
...
//! ## FEN dialect
//!
//! mce and FSF render the same Shatar position with **different Bers letters**.
//! FSF's `shatar` spells the Bers `j`; mce reuses `j` for its Xiangqi Horse role,
//! so the Bers takes the [`mce::geometry::WideRole::General`] letter `d` (the
//! Rook + Ferz role it shares with Spartan / Shinobi). [`to_fsf_dialect`] maps
//! mce's `d` / `D` back to FSF's `j` / `J` over the whole FEN. Only the placement
...
use mce::geometry::{perft as gperft, Chess8x8, Shatar};

use crate::uci::Engine;

/// One Shatar corpus position. The FEN is mce's dialect; the FSF side translates
...
/// Run the Shatar corpus through mce and FSF. Returns the number of mismatches
...
pub fn run(engine: &mut Engine, full: bool) -> usize {
    ...
    if mce_s > 0.0 && fsf_s > 0.0 {
        println!(
            "shatar OVERALL: {nodes} nodes verified; mce {:.1} Mn/s vs fsf {:.1} Mn/s ({:.2}x).",
    ...
    if mismatches == 0 {
        ...
        for r in rows.iter().filter(|r| !r.matched) {
            eprintln!(
                "  MISMATCH shatar/{} depth {}: mce={} fsf={}  FEN: {}",
...
/// Run one Shatar position through mce's generic perft and FSF's `go perft`.
fn run_case(engine: &mut Engine, case: &Case, depth: u32) -> Result<Row, String> {
    // mce side: the generic Shatar position (mce dialect).
    let pos = Shatar::from_fen(case.fen).map_err(|e| format!("mce rejected FEN: {e:?}"))?;
...
mod tests {
    ...
    /// The dialect swap maps mce's Bers letter `d` to FSF's `j` and leaves the
    /// standard army and structural fields untouched.
    #[test]
    fn dialect_swap_maps_bers_letter() {
        let mce = "rnbdkbnr/ppp1pppp/8/3p4/3P4/8/PPP1PPPP/RNBDKBNR w - - 0 1";
