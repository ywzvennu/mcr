...
//! (issue #266).
//!
//! Placement runs on mce's **generic** engine (`mce::geometry::Placement`, a
...
//! are identical in mce and FSF, so no piece-letter translation is needed. mce
...
//! mce's rendering verbatim. So the FEN is passed to FSF **unchanged**. The
...
/// Run the Placement corpus through mce and FSF. Returns the number of mismatches
...
pub fn run(engine: &mut Engine, full: bool) -> usize {
    ...
    if mce_s > 0.0 && fsf_s > 0.0 {
        println!(
            "placement OVERALL: {nodes} nodes verified; mce {:.1} Mn/s vs fsf {:.1} Mn/s ({:.2}x).",
    ...
    if mismatches == 0 {
        ...
        for r in rows.iter().filter(|r| !r.matched) {
            eprintln!(
                "  MISMATCH placement/{} depth {}: mce={} fsf={}  FEN: {}",
...
/// Run one Placement position through mce's generic perft and FSF's `go perft`.
fn run_case(engine: &mut Engine, case: &Case, depth: u32) -> Result<Row, String> {
    // mce side: the generic Placement position.
    let pos = Placement::from_fen(case.fen).map_err(|e| format!("mce rejected FEN: {e:?}"))?;
