...
/// * **Supply** — 9x10 Xiangqi-with-drops; an FSF built-in, but the available FSF
///   binary is a non-large-board build that has neither `supply` nor even `xiangqi`
///   (both fall back to standard chess), so there is no live FSF oracle in the
///   generic `SPECS` sweep. It has its own comparison loop ([`crate::supply`]) that,
///   on an FSF built `largeboards=yes`, drives the empty-hand Xiangqi equivalence
///   live against `UCI_Variant xiangqi`; the drop mechanic is cross-checked against
///   an independent from-scratch 9x10 generator. It is excluded from `SPECS`.
/// * **Wolf** — 8x10 Wolf chess is an FSF built-in, but the available FSF binary is a
///   non-large-board build that does not implement the 10-rank board (`setoption
///   UCI_Variant wolf` falls back to standard chess), so there is no usable oracle
///   here. It is rules-only, cross-checked against an independent from-scratch 8x10
///   generator; it carries no FSF spec.
...
mod tests {
    ...
    fn specs_are_well_formed() {
        ...
        // Every shipped variant minus the 13 by-design exclusions (Alice / Duck /
        // Jieqi, the HaChu-only large-shogi Chu / Dai / Tenjiku, and the oracle-less
        // Wa Shogi, Okisaki Shogi, Yari Shogi, Gustav 3, Omicron, Supply, and Wolf,
        // which the available Fairy-Stockfish build does not implement); the
        // deeper-sweep follow-ups stay in SPECS but are skipped via `HELD_BACK` on the
        // default run.
        assert_eq!(SPECS.len(), WideVariantId::ALL.len() - 13);
    }
