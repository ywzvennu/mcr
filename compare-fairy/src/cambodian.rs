...
//!
//! Cambodian runs on mcr's **generic** engine (`mcr::geometry::Cambodian`, a
...
use mcr::geometry::{perft as gperft, Cambodian, Chess8x8};
...
/// Run the Cambodian corpus through mcr and FSF. Returns the number of
...
pub fn run(engine: &mut Engine, full: bool) -> usize {
    ...
    if mcr_s > 0.0 && fsf_s > 0.0 {
        println!(
            "cambodian OVERALL: {nodes} nodes verified; mcr {:.1} Mn/s vs fsf {:.1} Mn/s ({:.2}x).",
    ...
    if mismatches == 0 {
        ...
        for r in rows.iter().filter(|r| !r.matched) {
            eprintln!(
                "  MISMATCH cambodian/{} depth {}: mcr={} fsf={}  FEN: {}",
...
/// Run one Cambodian position through mcr's generic perft and FSF's `go perft`.
fn run_case(engine: &mut Engine, case: &Case, depth: u32) -> Result<Row, String> {
    // mcr side: the generic Cambodian position.
    let pos = Cambodian::from_fen(case.fen).map_err(|e| format!("mcr rejected FEN: {e:?}"))?;
