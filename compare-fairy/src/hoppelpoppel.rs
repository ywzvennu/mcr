...
//!
//! Hoppel-Poppel runs on mce's **generic** engine (`mce::geometry::HoppelPoppel`, a
...
//! ## FEN dialect
//!
//! mce and FSF render the same Hoppel-Poppel position with **different letters** for
...
//! knight-captures); mce already names `n` the standard Knight and `b` the standard
...
/// One Hoppel-Poppel corpus position. The FEN is mce's dialect; the FSF side
...
/// Translates an mce-dialect Hoppel-Poppel FEN to FSF's dialect: rewrite the
...
pub(crate) fn to_fsf_dialect(fen: &str) -> String {
    ...
    let placement = parts
        .next()
        .unwrap_or("")
        // Knight-Bishop: mce `*h` → FSF `n` (both cases).
        .replace("*H", "N")
        .replace("*h", "n")
        // Bishop-Knight: mce `*b` → FSF `b` (both cases).
        .replace("*B", "B")
        .replace("*b", "b");
    match parts.next() {
...
/// Run the Hoppel-Poppel corpus through mce and FSF. Returns the number of
...
pub fn run(engine: &mut Engine, full: bool) -> usize {
    ...
    if mce_s > 0.0 && fsf_s > 0.0 {
        println!(
            "hoppelpoppel OVERALL: {nodes} nodes verified; mce {:.1} Mn/s vs fsf {:.1} Mn/s \
             ({:.2}x).",
    ...
    if mismatches == 0 {
        ...
        for r in rows.iter().filter(|r| !r.matched) {
            eprintln!(
                "  MISMATCH hoppelpoppel/{} depth {}: mce={} fsf={}  FEN: {}",
...
/// Run one Hoppel-Poppel position through mce's generic perft and FSF's `go perft`.
fn run_case(engine: &mut Engine, case: &Case, depth: u32) -> Result<Row, String> {
    let pos = HoppelPoppel::from_fen(case.fen).map_err(|e| format!("mce rejected FEN: {e:?}"))?;
...
mod tests {
    ...
    fn dialect_swap_maps_overflow_pieces() {
        let mce = "r*h*bqk*b*hr/pppppppp/8/8/8/8/PPPPPPPP/R*H*BQK*B*HR w KQkq - 0 1";
        assert_eq!(
            to_fsf_dialect(mce),
