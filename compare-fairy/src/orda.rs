...
//!
//! Orda runs on mce's **generic** engine (`mce::geometry::Orda`, a
...
//! ## FEN dialect
//!
//! mce and FSF render the same Orda position with **different Orda-piece letters**.
//! FSF's `orda` uses `l h a y` (Lancer = kniroo, Kheshig = centaur, Archer =
//! knibis, Yurt = silver); mce reuses `l`/`h`/`a` for its Lance / Hoplite / Hawk,
//! so the Orda pieces take distinct letters: Lancer `f`, Kheshig `w`, Archer `y`,
//! Yurt `s` (the existing Silver), King `k` (shared). [`to_fsf_dialect`] maps mce's
...
//! (mce Kheshig) → `H` (FSF centaur); the swap is unambiguous because the standard
...
/// One Orda corpus position. The FEN is mce's dialect; the FSF side translates it
/// via [`to_fsf_dialect`].
struct Case {
    label: &'static str,
    fen: &'static str,
...
/// Translates an mce-dialect Orda FEN to FSF's dialect by mapping the Orda piece
...
/// Run the Orda corpus through mce and FSF. Returns the number of mismatches
...
pub fn run(engine: &mut Engine, fsf_bin: &str, full: bool) -> usize {
    ...
    if mce_s > 0.0 && fsf_s > 0.0 {
        println!(
            "orda OVERALL: {nodes} nodes verified; mce {:.1} Mn/s vs fsf {:.1} Mn/s ({:.2}x).",
    ...
    if mismatches == 0 {
        ...
        for r in rows.iter().filter(|r| !r.matched) {
            eprintln!(
                "  MISMATCH orda/{} depth {}: mce={} fsf={}  FEN: {}",
...
/// Run one Orda position through mce's generic perft and FSF's `go perft`.
fn run_case(engine: &mut Engine, case: &Case, depth: u32) -> Result<Row, String> {
    let pos = Orda::from_fen(case.fen).map_err(|e| format!("mce rejected FEN: {e:?}"))?;
...
mod tests {
    ...
    /// The dialect swap maps mce's Orda letters to FSF's and leaves the standard
    /// army and structural fields untouched.
    #[test]
    fn dialect_swap_maps_orda_letters() {
        let mce = "fwyskywf/8/pppppppp/8/8/8/PPPPPPPP/RNBQKBNR w KQ - 0 1";
        let fsf = to_fsf_dialect(mce);
        assert_eq!(
            fsf,
            "lhaykahl/8/pppppppp/8/8/8/PPPPPPPP/RNBQKBNR w KQ - 0 1"
        );
        // A White-promoted Kheshig (mce `W`) maps to FSF's centaur `H`.
