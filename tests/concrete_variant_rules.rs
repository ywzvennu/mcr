...
/// `VariantRef::ALL` spans both families: the nine concrete plus the one-hundred-four wide.
#[test]
fn all_spans_both_families() {
    ...
    assert_eq!(wide, 108, "one hundred eight wide variants");
    assert_eq!(
        VariantRef::ALL.len(),
        117,
        "one hundred seventeen variants in all"
    );
}
